...
浏览器打开 http://127.0.0.1:5173 ，使用登录页进入系统。

**演示账号（密码均为 `123456`）**

| 登录名 | 显示名 | 角色 | 用途 |
|---|---|---|---|
| `admin` | admin | 领导 | 全部权限（含审批、基础数据） |
| `lizz` / `wangzg` / `zhaojl` | 李组长 / 王主管 / 赵经理 | 领导 | 三级审批人 |
| `zhangyw` | 张运维 | 主业运维 | 入库/装机/借出/调拨/报废/盘点（不可审批） |
| `wawei` | 万维 | 外委运维 | 与主业运维权限对等（不可审批） |
| `qiancg` | 钱仓管 | 设备供应商 | 仅分类入库（不可看配件列表/可调余量） |

也可用显示名登录（如 `张运维`）。多级审批需**退出后换领导账号**再登录。
